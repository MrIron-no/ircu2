/*
 * IRC - Internet Relay Chat, ircd/ircd_config.c
...
/** @file
 * @brief Generic server configuration system implementation.
 */
...
{
  struct ConfigEntry *entry = config_list;
  struct ConfigEntry *next;
  struct ConfigCallback *cb = callback_list;
  struct ConfigCallback *cb_next;
...
int config_set(const char *key, const char *value, time_t timestamp)
{
  ...
  Debug((DEBUG_INFO, "Config %s: %s = %s (timestamp: %lu)", 
...
{
  struct ConfigEntry *entry, *prev = NULL;
  
  assert(key != NULL);
  
  for (entry = config_list; entry; prev = entry, entry = entry->next) {
    if (ircd_strcmp(entry->key, key) == 0) {
      if (prev)
        prev->next = entry->next;
      else
        config_list = entry->next;
        
      MyFree(entry->key);
      MyFree(entry->value);
      MyFree(entry);
      
      Debug((DEBUG_INFO, "Config removed: %s", key));
      return;
    }
  }
}
...
void config_register_callback(const char *key_prefix, config_callback_f callback)
{
  ...
  Debug((DEBUG_INFO, "Config callback registered for prefix: %s", key_prefix));
}
...
void config_unregister_callback(const char *key_prefix)
{
  ...
  for (cb = callback_list; cb; prev = cb, cb = cb->next) {
    if (ircd_strcmp(cb->key_prefix, key_prefix) == 0) {
      ...
      Debug((DEBUG_INFO, "Config callback unregistered for prefix: %s", key_prefix));
      return;
    }
  }
}
...
void config_stats(struct Client *sptr, const struct StatDesc *sd, char *param)
{
  ...
  if (count == 0) {
    ...
    send_reply(sptr, SND_EXPLICIT | RPL_STATSDEBUG,
               ":Total configuration entries: %d", count);
  }
}